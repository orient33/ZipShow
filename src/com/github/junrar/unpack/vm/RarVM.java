...
public class RarVM extends BitInput {
	...
	private boolean ExecuteCode(List<VMPreparedCommand> preparedCode,
		...
		while (true) {
			...
			case VM_MOV:
				setValue(cmd.isByteMode(), mem, op1,
						getValue(cmd.isByteMode(), mem, op2)); // SET_VALUE(Cmd->ByteMode,Op1,GET_VALUE(Cmd->ByteMode,Op2));
				break;
			case VM_MOVB:
			...
			case VM_ADD: {
				int value1 = getValue(cmd.isByteMode(), mem, op1);
				int result = (int) ((((long) value1 + (long) getValue(
						cmd.isByteMode(), mem, op2))) & 0xffffffff);
				if (cmd.isByteMode()) {
					result &= 0xff;
			...
			case VM_PUSH:
				R[7] -= 4;
				setValue(false, mem, R[7] & VM_MEMMASK,
						getValue(false, mem, op1));
				break;
			case VM_POP:
				setValue(false, mem, op1,
						getValue(false, mem, R[7] & VM_MEMMASK));
				R[7] += 4;
				break;
			case VM_CALL:
				R[7] -= 4;
			...
			case VM_NEG: {
				...
				flags = result == 0 ? VMFlags.VM_FZ.getFlag() : VMFlags.VM_FC
						.getFlag() | (result & VMFlags.VM_FS.getFlag());
				setValue(cmd.isByteMode(), mem, op1, result);
			}
				break;
	...
	private VMStandardFilters IsStandardFilter(byte[] code, int codeSize) {
		...
		for (int i = 0; i < stdList.length; i++) {
			if (stdList[i].getCRC() == CodeCRC
					&& stdList[i].getLength() == code.length) {
	...
	private void ExecuteStandardFilter(VMStandardFilters filterType) {
		...
		case VMSF_E8E9: {
			...
			byte cmpByte2 = (byte) ((filterType == VMStandardFilters.VMSF_E8E9) ? 0xe9
					: 0xe8);
		...
		case VMSF_ITANIUM: {
			...
			int curPos = 0;
			final byte Masks[] = { 4, 4, 6, 6, 0, 0, 7, 7, 4, 4, 0, 0, 4, 4, 0,
					0 };
			fileOffset >>>= 4;
				...
				if (Byte >= 0) {
					...
					if (cmdMask != 0)
						for (int i = 0; i <= 2; i++)
							if ((cmdMask & (1 << i)) != 0) {
								int startPos = i * 41 + 5;
								int opType = filterItanium_GetBits(curPos,
										startPos + 37, 4);
								if (opType == 5) {
									int offset = filterItanium_GetBits(curPos,
											startPos + 13, 20);
	...
	private void filterItanium_SetBits(int curPos, int bitField, int bitPos,
			int bitCount) {
		int inAddr = bitPos / 8;
		int inBit = bitPos & 7;
